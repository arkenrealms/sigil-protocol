...
- Added whitespace-trim + case-normalization for `orderBy` directions (`ASC`/`DESC` and padded values like `" DESC "` -> `asc`/`desc`) to reduce avoidable caller failures while preserving strict rejection for unknown values.
